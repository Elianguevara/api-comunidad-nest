...
export class NotificationsService {
  ...
  async notifyProvidersByProfessionAndCity(idProfession: number, idCity: number, petition: Petition) {
  ...
  async getUnreadCount(email: string): Promise<number> {
    const user = await this.userRepo.findOne({ where: { email } });
    return this.notificationRepo.count({ where: { user: { idUser: user?.idUser }, isRead: false } });
  }
  ...
  async markAsRead(id: number, email: string) {
    ...
    if (notification.user.email !== email) throw new ForbiddenException('No tienes permiso');
  ...
  private async createNotification(user: User, title: string, message: string, type: string, link: string, petition?: Petition, postulation?: Postulation) {
